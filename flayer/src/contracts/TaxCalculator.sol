...
contract TaxCalculator is ITaxCalculator {
    ...
    function calculateCompoundedFactor(uint _previousCompoundedFactor, uint _utilizationRate, uint _timePeriod) public view returns (uint compoundedFactor_) {
        ...
        // interestRate is in basis points per annum with 1e2 precision
        // Converting annual rate to per second rate: interestRate / (365 * 24 * 60 * 60)
        uint perSecondRate = (interestRate * 1e18) / (365 * 24 * 60 * 60 * 1000);

        // Calculate new compounded factor
        compoundedFactor_ = _previousCompoundedFactor * (1e18 + (perSecondRate * _timePeriod)) / 1e18;
    }
